...
static uint8_t* g_draw_fb = (uint8_t*)0;
...
static uint8_t g_fb_dirty = 0;
static uint32_t g_dirty_min_x = 0;
static uint32_t g_dirty_min_y = 0;
static uint32_t g_dirty_max_x = 0;
static uint32_t g_dirty_max_y = 0;

#define FB_BACKBUFFER_MAX_BYTES (12U * 1024U * 1024U)
...
static void fb_mark_dirty_pixel(uint32_t x, uint32_t y)
{
    if (!g_fb_dirty) {
        g_dirty_min_x = x;
        g_dirty_min_y = y;
...
static void fb_mark_dirty_rect(uint32_t x, uint32_t y, uint32_t w, uint32_t h)
{
    uint32_t x2;
    uint32_t y2;

    if (w == 0 || h == 0 || x >= g_fb_width || y >= g_fb_height) {
        return;
    }

    x2 = x + w - 1;
    y2 = y + h - 1;
    if (x2 >= g_fb_width) x2 = g_fb_width - 1;
    if (y2 >= g_fb_height) y2 = g_fb_height - 1;
...
void graphics_init(uint32_t multiboot_magic, uint32_t multiboot_info_addr)
{
    ...
    g_use_backbuffer = 0;
    if (g_fb_enabled) {
        uint32_t need = g_fb_pitch * g_fb_height;
        if (need <= FB_BACKBUFFER_MAX_BYTES) {
            g_draw_fb = g_backbuffer;
            g_use_backbuffer = 1;
        }
    }
}
...
{
    if (!g_fb_enabled || !g_use_backbuffer || !g_fb_dirty) {
        return;
    }

    {
        uint32_t bpp_bytes = (g_fb_bpp == 24) ? 3U : 4U;
        uint32_t row_start = g_dirty_min_x * bpp_bytes;
        uint32_t row_end = (g_dirty_max_x + 1U) * bpp_bytes;

        for (uint32_t y = g_dirty_min_y; y <= g_dirty_max_y; ++y) {
            uint8_t* src = g_backbuffer + y * g_fb_pitch + row_start;
            uint8_t* dst = g_fb + y * g_fb_pitch + row_start;
            for (uint32_t x = row_start; x < row_end; ++x) {
                dst[x - row_start] = src[x - row_start];
            }
        }
    }
