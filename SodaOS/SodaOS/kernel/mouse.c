...
#define MOUSE_INVERT_X 0
#define MOUSE_INVERT_Y 0
#define MOUSE_SENSITIVITY_NUM 3
#define MOUSE_SENSITIVITY_DEN 1
#define MOUSE_ACCEL_THRESHOLD 6
#define MOUSE_ACCEL_NUM 2
#define MOUSE_ACCEL_DEN 1
...
void mouse_poll(mouse_state_t* out_state)
{
    ...
    for (uint32_t n = 0; n < 64; ++n) {
        ...
        // Base sensitivity scale.
        dx = (dx * MOUSE_SENSITIVITY_NUM) / MOUSE_SENSITIVITY_DEN;
        dy = (dy * MOUSE_SENSITIVITY_NUM) / MOUSE_SENSITIVITY_DEN;

        // Light acceleration for larger moves so cursor doesn't feel sluggish.
        if (dx >= MOUSE_ACCEL_THRESHOLD || dx <= -MOUSE_ACCEL_THRESHOLD) {
            dx = (dx * MOUSE_ACCEL_NUM) / MOUSE_ACCEL_DEN;
        }
        if (dy >= MOUSE_ACCEL_THRESHOLD || dy <= -MOUSE_ACCEL_THRESHOLD) {
            dy = (dy * MOUSE_ACCEL_NUM) / MOUSE_ACCEL_DEN;
        }
